...
/* name: accum
 * function(a,b,c,d,outTable) */
static int lcf1_accum (lua_State * L) {
...
static int lcf_main (lua_State * L) {
  ...
  /* function accum(a,b,c,d,outTable)
   * 	local t1= a + b
   * 	local t2 = t1 + c
   * 	outTable.out = t2 + d
   * end */
  lua_pushcfunction(L,lcf1_accum);
  lua_setfield(L,LUA_ENVIRONINDEX,"accum");
  ...
  /* accum(1,2,3,4,output) */
  lua_getfield(L,LUA_ENVIRONINDEX,"accum");
  lua_pushnumber(L,1);
  lua_pushnumber(L,2);
  lua_pushnumber(L,3);
  lua_pushnumber(L,4);
  lua_getfield(L,LUA_ENVIRONINDEX,"output");
  lua_call(L,5,0);
  assert(lua_gettop(L) - lc_nextra == 0);
